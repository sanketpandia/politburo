...
// CacheJob syncs live flight data with intelligent polling based on flight phase
...
func NewCacheJob(liveAPISvc *common.LiveAPIService, redisSvc *cache.RedisCacheService, vaConfigSvc *common.VAConfigService, logger *zap.SugaredLogger) *CacheJob {
	return &CacheJob{
...
func (j *CacheJob) determineFlightPhase(ctx context.Context, flightID string, speed, altitude float64) FlightPhase {
...
func (j *CacheJob) calculateNextPollTime(phase FlightPhase, now time.Time) time.Time {
	switch phase {
...
func (j *CacheJob) RunScheduled(ctx context.Context, interval time.Duration) {
