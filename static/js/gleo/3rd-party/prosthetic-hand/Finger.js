...
export default class Finger {
	...
	constructor(eventMode, options) {
		...
		this._state = capabilities.assign({}, {
		...
		this._finalState = capabilities.assign({}, this._state);
...
// 			until: this._movesUntil + delay
	...
	_queueMove( move ) {
		...
		this._finalState = move.finalState = capabilities.assign({}, this._finalState, move.finalState);
	...
	*/
	getEvents(timestamp, justOne) {
		...
		var changed = false;
		var previousState = capabilities.assign({}, this._state);
		...
		while (this._movements.length && this._movements[0].until <= now && !(changed && justOne)) {
			...
			capabilities.assign(this._state, done.finalState);
		...
		if (this._movements.length && !(changed && justOne)) {
			...
			if (updatedState && !this._statesAreEqual(updatedState, this._state)) {
				changed = true;
				capabilities.assign(this._state, updatedState);
			}
